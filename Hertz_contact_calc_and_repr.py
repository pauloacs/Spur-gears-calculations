...
plt.plot(Rho_2,sigma_H1, lw = 1.5,label='1 pair of teeth in contact ') 
plt.plot(Rho_2,sigma_H2, lw = 1.5,label='2 pairs of teeth in contact')
...
plt.annotate('\u03C3H[I]', xy =(T2I, sigma(Fn,T2I)[1]) , xycoords = 'data', xytext = (-90, +30),  textcoords = 'offset points',  fontsize = 10, color = 'b',arrowprops = dict(arrowstyle = '-|>', color = 'b',connectionstyle = 'arc3,rad=.2'))
plt.annotate('\u03C3H[W]', xy =(T2W, sigma(Fn,T2W)[1]) , xycoords = 'data', xytext = (30, 30),  textcoords = 'offset points',  fontsize = 10, color = 'b',arrowprops = dict(arrowstyle = '-|>', color = 'b',connectionstyle = 'arc3,rad=.2'))

col=list(np.arange(T2A,T2B+0.01,0.01))
sigma_Hb=[]
sigma_Hc1=[]
sigma_Hc2=[]
for rho_2 in col:                             
    rho_r,sigma_2=sigma(Fn/2,rho_2)                                               
    sigma_Hb.append(sigma_2)
col=np.array(col)
sigma_Hb=np.array(sigma_Hb)

colc=list(np.arange(T2V,T2W+0.01,0.01))
for rho_2 in colc:
    rho_r,sigma_1=sigma(Fn,rho_2)  
    rho_r,sigma_2=sigma(Fn/2,rho_2)                                                   
    sigma_Hc1.append(sigma_1)
    sigma_Hc2.append(sigma_2)
    
    
colc=np.array(colc)
sigma_Hc1=np.array(sigma_Hc1)
sigma_Hc2=np.array(sigma_Hc2)

#plt.xlabel('<--Gearing line-->', fontsize = 10, labelpad = 10,color="Blue")
plt.ylabel('Contact pressure '+'(\u03C3H)' , fontsize = 10, labelpad = 0, color="k")
plt.axis([0,T1T2,0,sigma_H2[-5]])
#plt.title('Tensão de contacto'.upper(), y = 1.1, fontsize = 14, color = 'purple', fontweight = 'bold')
plt.legend()
plt.fill_between(col, 0, sigma_Hb, color='blue', alpha='0.2', where=None, interpolate=False, step=None)
plt.fill_between(colc, sigma_Hc2, sigma_Hc1, color='blue', alpha='0.2', where=None, interpolate=False, step=None)
plt.show()
